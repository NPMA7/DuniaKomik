...
const UpdateChapters = () => {
  ...
  const id = pathname.split("/").pop(); // Mengambil ID dari URL, anggap formatnya seperti '/update-chapters/[id]'
    ...
    const fetchKomik = async () => {
      if (id) {
        ...
        if (foundKomik) {
          ...
          const nextChapterNumber = foundKomik.chapters.length + 1;
          // Mengatur tanggal terbit berdasarkan tanggal saat ini
          const today = new Date();
          const formattedDate = today.getFullYear() + '-' + (today.getMonth() + 1).toString().padStart(2, '0') + '-' + today.getDate().toString().padStart(2, '0');
          // Mengatur judul bab berdasarkan judul komik dan nomor bab
          const chapterTitle = `Chapter ${nextChapterNumber} - ${foundKomik.title}`;
          setNewChapter({ chapter_number: nextChapterNumber.toString(), posted_on: formattedDate, title: chapterTitle, file: "" });
        }
      }
    };
  ...
  const handleSubmit = async (event) => {
    ...
    if (id && komik) {
        const updatedChapters = chapters.map(chapter => chapter.chapter_number === newChapter.chapter_number ? newChapter : chapter);
        const updatedKomik = {
        ...
        if (response.ok) {
            ...
            setEditingChapter(null); // Mengatur ulang state editingChapter setelah submit
            // Mengatur ulang nomor bab berikutnya dan tanggal terbit setelah submit
            const nextChapterNumber = updatedChapters.length + 1;
            const today = new Date();
            const formattedDate = today.getFullYear() + '-' + (today.getMonth() + 1).toString().padStart(2, '0') + '-' + today.getDate().toString().padStart(2, '0');
            const chapterTitle = `Chapter ${nextChapterNumber} - ${komik.title}`;
            setNewChapter({ chapter_number: nextChapterNumber.toString(), posted_on: formattedDate, title: chapterTitle, file: "" });
        } else {
      ...
      <div className="w-1/2 pl-4">
        <form onSubmit={handleSubmit} className="flex flex-col">
          ...
          <div className="mb-4">
            ...
            <input type="date" id="posted_on" name="posted_on" value={newChapter.posted_on} onChange={handleInputChange} required className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" />
          </div>
          <div className="mb-4">
